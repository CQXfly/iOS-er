...
class MarkDownView: UIView {
    ...
    private func setAlink(text:String,href:String,lastV:inout UIView){
        ...
        if(lastV.isKind(of: YYLabel.classForCoder())){
            ...
            attr.setTextHighlight(range, color: UIColor.red, backgroundColor: UIColor.gray) { (view, str, range, rect) in
                print(text)
            }
            ...
            attr.setTextHighlight(NSMakeRange(0, text.count), color: UIColor.red, backgroundColor: UIColor.gray) { (view, str, range, rect) in
                print(str)
            }
